...
final class InvController
{
     ...
     *
     * @psalm-return array{number: null|string, 
     * id: null|string, 
     * user_id: string, 
     * client_id: string, 
     * date_created: \DateTimeImmutable|null, 
     * date_modified: \DateTimeImmutable, 
     * date_due: \DateTimeImmutable|null, 
     * group_id: string, 
     * status_id: int, creditinvoice_parent_id: null|string, 
    ...
    { 
        ...
        if ($ajax_content->load($ajax_body) && $validator->validate($ajax_content)->isValid()) {    
            $this->inv_service->addInv($this->user_service->getUser(),$inv,$ajax_content, $this->sR);
    ...
    public function create_credit_confirm(Request $request, ValidatorInterface $validator,IR $iR, GR $gR, IAR $iaR, IIR $iiR, IIAR $iiaR) : \Yiisoft\DataResponse\DataResponse {
            ...
            $new_inv = new Inv();
            if ($ajax_content->load($ajax_body) && $validator->validate($ajax_content)->isValid()) {    
                $this->inv_service->saveInv($this->user_service->getUser(),$new_inv,$ajax_content, $this->sR, $gR);
                $this->inv_item_service->initializeCreditInvItems((int)$basis_inv_id, $new_inv->getId(), $iiR,$iiaR, $this->sR);
                $this->inv_amount_service->initializeCreditInvAmount(new InvAmount(), (int)$basis_inv_id, $new_inv->getId() );
                $this->inv_tax_rate_service->initializeCreditInvTaxRate((int)$basis_inv_id, $new_inv->getId());
                $parameters = ['success'=>1];
                //return response to inv.js to reload page at location
                return $this->factory->createResponse(Json::encode($parameters));          
            } else {
    ...
    public function edit_check_status_reconciling_with_balance(IAR $iaR, int $inv_id) : bool {
      ...
      * @return InvForm
      */
     public function edit_save_form_fields(array|object|null $edited_body, CurrentRoute $currentRoute, ValidatorInterface $validator, IR $invRepo, GR $groupRepo) : InvForm {
        $form = new InvForm();
        if ($form->load($edited_body) && $validator->validate($form)->isValid()) {
                $this->inv_service->saveInv($this->user_service->getUser(),$this->inv($currentRoute, $invRepo, true),$form, $this->sR, $groupRepo);
        }
        return $form;
    }
    ...
    public function email_get_invoice_templates(string $type = 'pdf')
    {
                                  ...
                                  PCR $pcR, QCR $qcR, UIR $uiR) : Response 
    {
        ...
        $inv_id = $this->inv($currentRoute, $iR, true)->getId();
                                  ...
                                  ViewRenderer $viewrenderer) : bool
    {
        ...
        $inv_custom_values = $this->inv_custom_values($inv_id, $icR);
        $inv = $iR->repoCount((string)$inv_id) > 0 ? $iR->repoInvUnLoadedquery($inv_id) : null;
        $stream = false;
     ...
     * @return InvItem|Response
     */
    private function inv_item(CurrentRoute $currentRoute, IIR $invitemRepository) : InvItem|Response 
    {
        $id = $currentRoute->getArgument('id');       
    ...
    /**
     * @param null|string $copy_id
     */
    private function inv_to_inv_inv_amount(string $inv_id,string|null $copy_id): void {
     ...
     * @param null|string $copy_id
     */
    private function inv_to_inv_inv_custom(string $inv_id, string|null $copy_id, ICR $icR, ValidatorInterface $validator): void {
     ...
     * @param null|string $copy_id
     */
    private function inv_to_inv_inv_items(string $inv_id, string|null $copy_id, IIAR $iiaR, IIAS $iiaS, PR $pR, TASKR $taskR, IIR $iiR, TRR $trR, ValidatorInterface $validator, UNR $unR): void {
        ...
        foreach ($items as $inv_item) {
           ...
           if ($form->load($copy_item) && $validator->validate($form)->isValid()) {
                !empty($inv_item->getProduct_id()) && empty($inv_item->getTask_id()) ? 
     ...
     * @param null|string $copy_id
     */
    private function inv_to_inv_inv_tax_rates(string $inv_id, string|null $copy_id, ITRR $itrR, ValidatorInterface $validator): void {
     ...
     * @param CurrentRoute $currentRoute
     * @param ITRR $invtaxrateRepository
     * @return InvTaxRate|Response
     */
    
    private function invtaxrate(CurrentRoute $currentRoute, ITRR $invtaxrateRepository): InvTaxRate|Response 
    {
        $id = $currentRoute->getArgument('id');       
    ...
    public function url_key(CurrentRoute $currentRoute, CFR $cfR, IAR $iaR, IIAR $iiaR, IIR $iiR, IR $iR, ITRR $itrR, UIR $uiR, PMR $pmR, UPR $upR): Response 
    {
        $url_key = $currentRoute->getArgument('url_key');
        if ($url_key === null) {
            return $this->web_service->getNotFoundResponse();
        ...
        $uiR->repoUserInvUserIdcount($inv->getUser_id()) === 1 && $uiR->repoUserInvUserIdquery($inv->getUser_id())->getType() !== 1 && $inv->getStatus_id() === 2 ? $inv->setStatus_id(3) : '';
        $iR->save($inv);
        ...
        $is_overdue = (null!==$inv_amount && $inv_amount->getBalance() > 0 && ($inv->getDate_due()) < (new \DateTimeImmutable('now')) ? true : false);
    ...
    public function view(ViewRenderer $head, CurrentRoute $currentRoute, Request $request,
        ...
        $inv_amount = (($iaR->repoInvAmountCount((int)$this->inv($currentRoute, $iR)->getId()) > 0) ? $iaR->repoInvquery((int)$this->session->get('inv_id')) : null);
        $inv_custom_values = $this->inv_custom_values($this->session->get('inv_id'), $icR);
        ...
        $url_key = $this->inv($currentRoute, $iR, false)->getUrl_key();
        $client_id = $this->inv($currentRoute, $iR, false)->getClient_id();
        $parameters = [
    ...
    private function view_partial_item_table(bool $show_buttons, CurrentRoute $currentRoute, PR $pR, TaskR $taskR, IIR $iiR, IIAR $iiaR, IR $iR, TRR $trR, UNR $uR, ITRR $itrR, InvAmount $inv_amount) : string {
