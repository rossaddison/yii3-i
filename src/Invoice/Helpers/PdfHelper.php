...
Class PdfHelper
{
    ...
    /**
     * @param \App\Invoice\Entity\Inv|\App\Invoice\Entity\Quote|array $quote_or_inv
     */
    private function get_print_language(array|\App\Invoice\Entity\Quote|\App\Invoice\Entity\Inv $quote_or_inv){
     ...
     * @param bool $stream
     * @param bool $custom
     * @param \App\Invoice\Entity\QuoteAmount|null $quote_amount
     ...
     * @return string
     */
    public function generate_quote_pdf(string|null $quote_id, string $user_id, bool $stream, bool $custom, \App\Invoice\Entity\QuoteAmount|null $quote_amount, array $quote_custom_values,\App\Invoice\Client\ClientRepository $cR, \App\Invoice\CustomValue\CustomValueRepository $cvR, \App\Invoice\CustomField\CustomFieldRepository $cfR, \App\Invoice\QuoteItem\QuoteItemRepository $qiR, \App\Invoice\QuoteItemAmount\QuoteItemAmountRepository $qiaR, \App\Invoice\Quote\QuoteRepository $qR, \App\Invoice\QuoteTaxRate\QuoteTaxRateRepository $qtrR, \App\Invoice\UserInv\UserInvRepository $uiR,
                                \Yiisoft\Yii\View\ViewRenderer $viewrenderer) : string
    {       
        $quote = $qR->repoCount((string)$quote_id) > 0 ? $qR->repoQuoteLoadedquery((string)$quote_id) : null;
        ...
        $items = ($qiR->repoCount((string)$quote_id) > 0 ? $qiR->repoQuoteItemIdquery($quote_id) : null);
     ...
     * @param bool $stream
     * @param bool $custom
     * @param \App\Invoice\Entity\InvAmount|null $inv_amount
     ...
     * @return string
     */
    public function generate_inv_pdf(string $inv_id, string $user_id, bool $stream, bool $custom, \App\Invoice\Entity\InvAmount|null $inv_amount, array $inv_custom_values,\App\Invoice\Client\ClientRepository $cR, \App\Invoice\CustomValue\CustomValueRepository $cvR, \App\Invoice\CustomField\CustomFieldRepository $cfR, \App\Invoice\InvItem\InvItemRepository $iiR, \App\Invoice\InvItemAmount\InvItemAmountRepository $iiaR, \App\Invoice\Inv\InvRepository $iR, \App\Invoice\InvTaxRate\InvTaxRateRepository $itrR, \App\Invoice\UserInv\UserInvRepository $uiR,
                                \Yiisoft\Yii\View\ViewRenderer $viewrenderer) : string
    {       
