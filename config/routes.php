    ...
    Group::create('/invoice')
        ->routes(
            ...
            Route::methods([Method::GET, Method::POST], '/generator/quick_view_schema')
             ...
             Route::methods([Method::GET, Method::POST], '/generator/google_translate/{type}')
                ->name('generator/google_translate')
                ->middleware(fn (AccessChecker $checker) => $checker->withPermission('editInv'))
                ->middleware(Authentication::class)
                ->action([GeneratorController::class, 'google_translate']),    
            ...
            // The individual that is sent the quote either approves or rejects it    
            Route::methods([Method::GET, Method::POST], '/quote/approve/{url_key}')
                ->name('quote/approve')
                ->middleware(fn (AccessChecker $checker) => $checker->withPermission('viewInv'))
                ->middleware(Authentication::class)
