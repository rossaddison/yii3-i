...
<form id="generatorForm" method="POST" action="<?= $urlGenerator->generate(...$action) ?>" enctype="multipart/form-data">
  ...
  <div class="container">
    <div class="row">
      <div class="col card mb3">
          ...
          <select name="pre_entity_table" id="pre_entity_table" class="form-control" required>
      ...
      <div class="col card mb3">
          ...
          <label for="namespace_path" class="form-label">Namespace before Entity Path eg. App\Invoice (NOT App\Invoice\Entity) </label>
          <input type="text" class="form-control" required name="namespace_path" id="namespace_path" placeholder="Namespace Path" value="<?= Html::encode($body['namespace_path'] ?? 'App\Invoice') ?>" required>
      </div>
    </div>
  </div>
  <br>
  ...
  <div class="card row mb-3">
    ...
    <div class="col mb-3">
        <label for="route_prefix" class="form-label">Route Prefix eg. invoice in 'invoice/product' that will appear after the controller construct.</label>
        <input type="text" class="form-control" name="route_prefix" id="route_prefix" placeholder="Route Prefix eg. invoice in 'invoice/product' that will appear after the controller construct." value="<?= Html::encode($body['route_prefix'] ?? 'invoice') ?>" required>
    </div>
    <div class="col mb-3">
        <label for="route_suffix" class="form-label">Route Suffix eg. product in 'invoice/product' that will appear after the controller construct.</label>
        <input type="text" class="form-control" name="route_suffix" id="route_suffix" placeholder="Route Suffix eg. product in 'invoice/product' that will appear after the controller construct." value="<?= Html::encode($body['route_suffix'] ?? 'product') ?>" required>
    </div>
    <div class="col mb-3">
        <label for="camelcase_capital_name" class="form-label">Camelcase Capital Name used in Controller and Repository names eg. TaxRate. Use 'Product' if using above example of 'product'. </label>
    ...
    <div class="col mb-3">
        <label for="small_singular_name" class="form-label">Small <b>singular</b> name used in Controller for <b>edit</b>, and <b>view</b> controller functions. Normally the same as the Route Suffix. eg. product</label>
    ...
    <div class="col mb-3">
        <label for="small_plural_name" class="form-label">Small <b>plural</b> name  used in Controller for <b>index</b> controller function to list all entity generators. Normally the above value with an s on the end.</label>
  ...
  <div class="card row mb-3">
    ...
    <div class="col mb-3 form-group">     
        <label for="controller_layout_dir" class="form-label">Controller Layout Directory eg. dirname(dirname(__DIR__)) that appears just after controller construct. The Controller file sits in (__DIR__) and is two directories below 'src' directory which wil be used as a 'base' to append a path to Layout directory. </label>
  ...
  <div class="card row mb-3">
    ...
    <div class="col mb-3 form-group">      
        <div class="mb-3">
            <label for="repo_extra_camelcase_name" class="form-label">External Entity eg. MyEntity exclusive of path. Path built in Generator. Default: Setting</label>
            <input type="text" class="form-control" name="repo_extra_camelcase_name" id="repo_extra_camelcase_name" placeholder="External Entity eg. MyEntity exclusive of path. Path built in Generator. Default: Setting. Additional Repository eg. Setting Repository in addition to main repository." value="<?= Html::encode($body['repo_extra_camelcase_name'] ?? 'Setting') ?>">
        </div>
    </div>
  </div>   
  <div class="container">
  ...
  <div class="container">  
    <div class="card row mb-3">
        ...
        <div class="col mb-3">
                <label for="paginator_next_page_attribute" class="form-label">Keyset Paginator next page attribute</label>
